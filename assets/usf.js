/* USF file - DO NOT MODIFY THIS FILE. THIS FILE IS REGULARLY CHANGED BY USF APP AND **ANY DIRECT CHANGES WILL BE LOST**. Use our in-app customization if you need to update CSS and JS code. Auto modified at: 2/6/2022 4:23:09 PM*/
/* Begin custom theme code */
// define templates for the Broadcast-Invisible-2.1.1 theme
...
<span v-else-if="hasDiscount && !isSoldOut && usf.settings.search.showSale" class="sale-box" v-html="_usfSaleText"></span>
<template v-if="_usfGlobalSettings.quickview_enable && !isSoldOut">
    <button type="button" class="quick-add-button" tabindex="-1" data-button-quick-add-mobile :data-quick-add-button="_usfProductHasOnlyDefaultVariant(product)">
    ...
    <div class="slideup is-not-relative" data-quick-add-holder>
        <button type="buttton" class="caps is-disable" data-focus-element :data-quick-add-label="product.urlName" :data-collection-handle="usf.platform.collection" :data-quick-add-button="_usfProductHasOnlyDefaultVariant(product)" tabindex="0">
            <span v-html="_usfQuickViewText"></span>
            <span v-html="_usfQuickViewAdd"></span>
...
<div class="usf-sr-container" :class="{'usf-no-facets': noFacets, 'usf-empty': !loader && !hasResults, 'usf-nosearch': !showSearchBox}">
    ...
    <div :class="(view === \'grid\' ? \'product-grid \' + (_usfGlobalSettings.product_grid_outline ? ' product-grid--borderless ' : '') : \'list-view-items\') + \' usf-results usf-clear usf-\' + view">
...
<div class="product-item" :class="[_usf_grid_strings,_usf_border_class,_usf_alignment_class,'product-item--collection-' + (productIndex+1)]" data-product-grid-item :data-usf-pid="product.id">
    <div class="product-item__image usf-sr-product__image-container" :class="{'double__image': hoverImage && _usfGlobalSettings.image_hover_enable}" data-product-image>
        <a data-grid-link class="product-link is-not-relative" :href="productUrl" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave" :aria-label="product.title">
            <!-- product image extra -->
            ...
            <template v-if="product.images.length">
                <div class="product-item__bg lazyload" data-grid-slide data-aos="img-in" :data-aos-delay="animation_delay" :data-aos-duration="productIndex*100+800" :data-aos-anchor="animation_anchor" data-aos-easing="ease-out-quart" :data-bgset="_usfGetBgset(selectedImage)"></div>
                <div v-if="hoverImage && _usfGlobalSettings.image_hover_enable" class="product-item__bg__under lazyload" :data-bgset="_usfGetBgset(hoverImage)"></div>
            </template>
            <template v-else>
                <div class="product-item__bg lazyload" data-grid-slide data-aos="img-in" :data-aos-delay="animation_delay" :data-aos-duration="productIndex*100+800" :data-aos-anchor="animation_anchor" data-aos-easing="ease-out-quart" :data-bgset="usf.platform.emptyImage.url"></div>
            </template>
        </a>
        `+ _usfOverImageContent +`
    </div>

    <div class="product-information" data-aos="fade" :data-aos-delay="animation_delay" :data-aos-duration="productIndex*50+800" :data-aos-anchor="animation_anchor" data-product-information>
        <a data-grid-link class="product-link" :href="productUrl" :aria-label="product.title">
            <!-- Product review -->
            <usf-plugin name="searchResultsProductReview" :data="pluginData"></usf-plugin>
            <!--product title-->
            <p class="product__grid__title" v-html="product.title"></p>
            <p>
            ...
            <span class="price" :class="{'sale':hasDiscount}">
                ...
                <template v-else>
                    ...
                    <span class="new-price" v-else v-html="displayDiscountedPrice"></span>
                    <span v-if="hasDiscount" class="old-price" v-html="priceVaries && !product.selectedVariantId ? displayMinPrice : displayPrice"></span>
                </template>
            </span>
            <template v-if="_usfGlobalSettings.color_swatches && !_usfGlobalSettings.quickview_enable">
                <span v-if="_has_colors" class="grid__product-swatches-count" data-swatch-count>&nbsp;</span>

                <form v-if="_has_colors" class="radio__fieldset radio__fieldset--swatches radio__fieldset--small" data-grid-swatch-form>
...
<center>
    <ul class="pagination-custom">
        <template v-for="e in elements">
            <li v-if="e.type === 'prev'" class="pagination-custom__arr">
                <a href="javascript:void(0)" :title="loc.prevPage" @click="onPrev">
                    <span class="visually-hidden">« Previous</span>
                </a>
            </li>
            <li v-else-if="e.type === 'dots'" class="usf-sr-pages__dots"><span>...</span></li>
            <li v-else-if="e.type === 'page' && e.current" class="pagination-custom__num active"><span><span>{{e.page}}</span></span></li>
            <li v-else-if="e.type === 'page' && !e.current" class="pagination-custom__num"><a href="javascript:void(0)" @click="ev=>onPage(e.page,ev)" :title="usf.utils.format(loc.gotoPage,e.page)">{{e.page}}</a></li>
            <li v-else-if="e.type === 'next'" class="pagination-custom__arr">
    ...
    usf.event.add(['sr_updated', 'sr_viewChanged', 'rerender'], function () {
        setTimeout(function () {
            if (window.Shopify && Shopify.theme
                ...
                && Shopify.theme.sections.registered.collection
                && Shopify.theme.sections.registered.collection.callStack
                && Shopify.theme.sections.registered.collection.callStack.onLoad) {
                var selector = usf.platform.collection ? '[data-section-type="collection"]' : '[data-section-type="search"]';
                var container = {
                    container: document.querySelector(selector),
                    id: 'collection',
                    type: 'collection'
                };
                _usfRegisterQuickAdd(container);
...
function _usfRegisterQuickAdd(container) {
    if (Shopify.theme.sections.registered.collection.callStack.onLoad[3]
        && Shopify.theme.sections.registered.collection.callStack.onLoad[3].apply)
        Shopify.theme.sections.registered.collection.callStack.onLoad[3].apply(container);
}
function _usfRegisterSwatchs(container) {
    if (_usfGlobalSettings.color_swatches
        && !_usfGlobalSettings.quickview_enable
        && Shopify.theme.sections.registered.collection.callStack.onLoad[6]
        && Shopify.theme.sections.registered.collection.callStack.onLoad[6].apply) {
        Shopify.theme.sections.registered.collection.callStack.onLoad[6].apply(container)
    }   
}
...
function _usfOnAddToCartSuccess(rs, form) {
    ...
    ), cart.settings.times.timeoutAddProduct)
}
